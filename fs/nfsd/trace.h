...
DECLARE_EVENT_CLASS(syno_nfsd_io_class,
	...
	TP_printk("xid=0x%08x fh_hash=0x%08x offset=%lld len=%lu latency=%lld client=[%s] ver=%d",
		  __entry->xid, __entry->fh_hash,
		  __entry->offset, __entry->len,
		  __entry->latency, __get_str(client_addr),
		  __entry->ver)
)
...
DEFINE_EVENT(syno_nfsd_io_class, syno_nfsd_##name,	\
	TP_PROTO(struct svc_rqst *rqstp,	\
		 ...
		 s64	latency,		\
		 char   *client_addr_str),	\
	TP_ARGS(rqstp, fhp, offset, len, latency, client_addr_str))
