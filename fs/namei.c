...
{
	if (unlikely(!nd->real_filename_cur_locate)) {
		WARN_ON_ONCE(1);
		return -EINVAL;
	}
...
static inline int update_path_to_real_filename(struct nameidata *nd)
{
	...
	if (LAST_ROOT == nd->last_type
			|| LAST_DOTDOT == nd->last_type
			|| LAST_DOT == nd->last_type)
		return 0;

	if (unlikely(!nd->caseless_path.mnt || !nd->caseless_path.dentry)) {
		WARN_ON_ONCE(1);
		return -EINVAL;
	}
	...
	if (nd->flags & LOOKUP_MOUNTED) {
		...
		target_name = mnt->mnt_mountpoint->d_name.name;
		target_len = mnt->mnt_mountpoint->d_name.len;
	} else {
		target_name = nd->caseless_path.dentry->d_name.name;
...
OK:
		...
		if (caseless_flag && !in_link) {
			err = update_path_to_real_filename(nd);
			if (err)
				return err;
...
static int path_lookupat(struct nameidata *nd, unsigned flags, struct path *path)
{
	...
	while (!(err = link_path_walk(s, nd))) {
		 ...
		 */
		if ((nd->flags & LOOKUP_CASELESS_COMPARE) && update_last) {
			update_last = false;
			err = update_path_to_real_filename(nd);
			if (err)
				return err;
...
static inline struct synotify_rename_path * get_rename_path(struct vfsmount *vfsmnt, struct dentry *old_dentry, struct dentry *new_dentry)
{
	...
	tmp_old_path = kstrdup(tmp_old_full_path, GFP_NOFS | __GFP_ZERO);
	if (!tmp_old_path) {
		goto end;
	}
	tmp_new_path = kstrdup(tmp_new_full_path, GFP_NOFS | __GFP_ZERO);
	if (!tmp_new_path) {
		goto end;
	}
...
struct synotify_rename_path * get_rename_path_list(struct dentry *old_dentry, struct dentry *new_dentry)
{
	...
	list_for_each(list_head, &mnt_space->list) {
		...
		if (!((FS_MOVED_FROM | FS_MOVED_TO) & mnt->mnt_fsnotify_syno_mask)) {
			continue;
		}
