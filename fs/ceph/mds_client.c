...
static int parse_reply_info_lookupsnap(void **p, void *end,
				  struct ceph_mds_reply_info_parsed *info, u64 features)
{
	ceph_decode_32_safe(p, end, info->snap_dname_len, bad);
	ceph_decode_need(p, end, info->snap_dname_len, bad);
	info->snap_dname = *p;
				  ...
				  u64 features, struct ceph_mds_session *s)
{
	...
	if (op == CEPH_MDS_OP_GETFILELOCK)
		...
		return parse_reply_info_lookupsnap(p, end, info, features);
#endif /* CONFIG_SYNO_CEPH_CASELESS_STAT */
	else
		return -EIO;
}
