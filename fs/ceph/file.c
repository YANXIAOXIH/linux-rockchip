		     ...
		     struct file *file, unsigned flags, umode_t mode)
{
	...
	if (flags & O_CREAT) {
		...
		err = ceph_pre_init_archive_bit(&as_ctx);
		if (err < 0)
			goto out_ctx;
...
static ssize_t __ceph_write_iter(struct kiocb *iocb, struct iov_iter *from,
				int is_recvfile, struct socket *sock, size_t *rbytes, size_t *wbytes);

static ssize_t ceph_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	return __ceph_write_iter(iocb, from, 0, NULL, NULL, NULL);
}
...
retry_snap:
#ifdef CONFIG_SYNO_CEPH_RECVFILE
	if (is_recvfile)
		ceph_start_io_write_nolock(inode); // inode is already locked in vfs_recvfile
	else
#endif /* CONFIG_SYNO_CEPH_RECVFILE */
	if (direct_lock)
		ceph_start_io_direct(inode);
	...
	if (is_recvfile) {
		...
		written = do_recvfile(file, sock, pos, count, rbytes, wbytes);

		if (likely(written >= 0))
			iocb->ki_pos = pos + written;
		ceph_end_io_write_nolock(inode);
	} else {
#endif /* CONFIG_SYNO_CEPH_RECVFILE */
...
#ifdef CONFIG_SYNO_CEPH_RECVFILE
static ssize_t ceph_recvfile(int fd, struct file *filp, struct socket *sock, loff_t pos,
			    size_t count, size_t * rbytes, size_t * wbytes)
{
	...
	return __ceph_write_iter(&iocb, &iter, 1, sock, rbytes, wbytes);
}
#endif /* CONFIG_SYNO_CEPH_RECVFILE */
