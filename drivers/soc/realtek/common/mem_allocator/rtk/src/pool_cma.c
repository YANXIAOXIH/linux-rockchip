...
#ifdef CONFIG_ARM64
		__dma_flush_area(ptr, flush_size);
#elif defined(CONFIG_ARM)
					    ...
					    unsigned long flags)
{
	...
	do {
		...
		if (rtk_flag_is_protected(common->rtk_flags) &&
			...
			page->page =
			    cma_alloc(pool->cpool, cma_count, cma_align,
				      GFP_KERNEL | GFP_DMA);
		}
			    ...
			    char *prefix)
{
	...
	do {
		char *space_prefix = "    ";
		char *sub_prefix =
		    kzalloc(strlen(prefix) + strlen(space_prefix), GFP_KERNEL);
		if (!sub_prefix)
			break;
