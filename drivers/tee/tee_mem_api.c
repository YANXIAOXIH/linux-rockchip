...
struct tee_mem_api_module * gModule = NULL;
static int __init tee_mem_api_module_init(void)
{
    int ret = -1;
    ...
    do {
        ...
        pr_info("%s%s : %d\n", TEE_MEM_TAG, __func__, __LINE__);
        ion_rtk_protected_notifier_register(&module->protected_notifier);
        pr_info("%s%s : %d\n", TEE_MEM_TAG, __func__, __LINE__);
...
static void __exit tee_mem_api_module_exit (void)
{
    if (gModule) {
        struct tee_mem_api_module * module = gModule;
        ion_rtk_protected_notifier_unregister(&module->protected_notifier);
        {
...
#ifdef CONFIG_ION_RTK_DHC_HEAP
subsys_initcall_sync(tee_mem_api_module_init);
#else
module_init(tee_mem_api_module_init);
#endif
module_exit(tee_mem_api_module_exit);
