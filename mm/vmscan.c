...
static int kswapd_per_node_run(int nid)
{
	...
	for (hid = 0; hid < kswapd_threads; ++hid) {
		...
		if (!pgdat->kswapd[0])
			memcpy(pgdat->kswapd, pgdat->mkswapd[hid], sizeof(pgdat->kswapd));
	}

	return ret;
}

static void __maybe_unused kswapd_per_node_stop(int nid)
{
	int hid = 0;
	...
	memset(NODE_DATA(nid)->kswapd, 0, sizeof(NODE_DATA(nid)->kswapd));
}
