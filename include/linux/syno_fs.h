...
static inline int syno_op_locker_state_get(const struct inode *inode, enum locker_state *state)
{
	if (!inode->i_op->syno_locker_state_get)
		return -EOPNOTSUPP;

	return inode->i_op->syno_locker_state_get((struct inode *)inode, state);
}
...
static inline bool syno_op_locker_is_appendable(const struct inode *inode)
{
	int ret;
	enum locker_state state;
...
static inline bool syno_op_locker_is_immutable(const struct inode *inode)
{
	int ret;
	enum locker_state state;
